...
static size_t log_event_size[numTotalWrappers] = {
  ...
  sizeof(log_event_fprintf_t),
  sizeof(log_event_fscanf_t),
                                 ...
                                 FILE* stream, const char* format, va_list ap)
{
  log_entry_t e = EMPTY_LOG_ENTRY;
  setupCommonFields(&e, clone_id, event);
  SET_FIELD(e, fprintf, stream);
  SET_FIELD(e, fprintf, format);
  return e;
}
                                ...
                                FILE* stream, const char* format, va_list ap)
{
  log_entry_t e = EMPTY_LOG_ENTRY;
  setupCommonFields(&e, clone_id, event);
  SET_FIELD(e, fscanf, stream);
  SET_FIELD(e, fscanf, format);
  return e;
}
...
int fprintf_turn_check(log_entry_t *e1, log_entry_t *e2)
{
  return base_turn_check(e1,e2)
    ...
    && ARE_FIELDS_EQUAL_PTR (e1, e2, fprintf, ap);
}
...
int fscanf_turn_check(log_entry_t *e1, log_entry_t *e2)
{
  return base_turn_check(e1,e2)
    ...
    && ARE_FIELDS_EQUAL_PTR (e1, e2, fscanf, ap);
}
